...
file = np.genfromtxt(r'C:\Users\user\Desktop\프로그래밍\projects\gesture HCI\data\gesture_train.csv', delimiter=',')
...
cv2.namedWindow('Game', cv2.WINDOW_NORMAL)  # 창 크기 변경 가능
        ...
        for res in result.multi_hand_landmarks:
            ...
            if len(rps_result) >= 2:
                ...
                if winner is not None:
                    cv2.putText(img, text='Winner', org=(rps_result[winner]['org'][0], rps_result[winner]['org'][1] + 70), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 255, 0), thickness=3)
                cv2.putText(img, text=text, org=(int(img.shape[1] / 2), 100), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 0, 255), thickness=3)
